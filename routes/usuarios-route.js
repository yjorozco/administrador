...
router.patch('/:id', ACL('admin_modificar_usuarios'), [check('nombre').not().isEmpty().trim().escape(), check('apellido').not().isEmpty().trim().escape(), check('direccion').not().isEmpty().trim().escape(), check('activo').not().isEmpty().trim().escape(), check('correo').escape().trim().isEmail(), check('password').trim().escape()], actualizarUsuario);
router.post('/cambio', ACL('admin_cambiar_password'), [check('password').not().isEmpty().trim().escape()], cambiarPassword);
